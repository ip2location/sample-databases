# IP2Location DB25 – IP Geolocation Database

## IP-Country-Region-City-Latitude-Longitude-ZIPCode-TimeZone-ISP-Domain-NetSpeed-AreaCode-Weather-Mobile-Elevation-UsageType-AddressType-Category Database Sample

[IP2Location DB25 database](https://www.ip2location.com/database/db25-ip-country-region-city-latitude-longitude-zipcode-timezone-isp-domain-netspeed-areacode-weather-mobile-elevation-usagetype-addresstype-category) provides an IP geolocation solution to determine the country, region or state, city, latitude and longitude, ZIP/Postal code, time zone, Internet Service Provider (ISP) or company name, domain name, net speed, area code, weather station code, weather station name, mobile country code (MCC), mobile network code (MNC) and carrier brand, elevation, usage type, address type and IAB category of IP address.

This repository offers a sample version of the IP2Location DB25 database in CSV, BIN, and CIDR formats. The sample includes a limited number of data rows and is intended solely for evaluation and testing—not for production use. For full, high‑accuracy data with daily updates, please subscribe to the commercial IP2Location DB25 database. 
...
| address_type           | CHAR(1)                       | IP address types as defined in IPv4 and IPv6: <br><br>A = Anycast (One to the closest)<br>U = Unicast (One to one)<br>M = Multicast (One to multiple)<br>B = Broadcast (One to all) |
| category               | VARCHAR(10)                   | The domain category based on IAB Tech Lab Content Taxonomy. Includes Tier-1 and Tier-2 (if available) level categories commonly used in advertising, Internet security, and filtering services. |
...
| **CIDR**     | [Download Sample](https://github.com/ip2location/sample-databases/tree/main/IP2Location/DB25/ip2location-db25-sample.ipv4.cidr.csv) | [Download Sample](https://github.com/ip2location/sample-databases/tree/main/IP2Location/DB25/ip2location-db25-sample.ipv6.cidr.csv) |
